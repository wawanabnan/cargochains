...
def get_customer_role_type():
    return PartnerRoleTypes.objects.get(code__iexact="customer")
...
class CustomerForm(forms.ModelForm):
    ...
    class Meta:
        ...
        # field geo model asli kita exclude karena kita pakai IntegerField proxy
        exclude = ["province", "regency", "district", "village", "location", "roles"]

        # ✅ RAMPING: tidak ada contact fields dan tidak ada company induk
        fields = [
            # mode
            "is_individual",
    ...
    def __init__(self, *args, **kwargs):
        ...
        # BALANCE: text (biar bisa format Indo), tapi tetap step untuk helper
        if "balance" in self.fields:
            self.fields["balance"].widget.attrs["step"] = "0.01"

        # ===== GEO (sama konsep stabil om) =====
    ...
    def clean_balance(self):
        raw = (self.cleaned_data.get("balance") or "").strip()
        if not raw:
            return Decimal("0.00")
        ...
        try:
            ...
            raise forms.ValidationError("Format balance tidak valid. Contoh: 1.500.000,00")
        return val
    ...
    def clean(self):
        ...
        # ✅ RULE WAJIB:
        # - personal: name wajib
        # - company: name wajib + company_name wajib
        ...
        # ❌ tidak pakai induk company untuk customer create/edit
    ...
    def save(self, commit=True):
        ...
        # ✅ Customer tidak punya induk company di create/edit
        if hasattr(instance, "company_id"):
            instance.company_id = None

        # ✅ personal customer otomatis contact sales+billing (kalau field ada di model)
        if getattr(instance, "is_individual", False):
            ...
            # company customer bukan contact person
            if hasattr(instance, "is_sales_contact"):
        ...
        if commit:
            ...
            # paksa role CUSTOMER
            role_type = get_customer_role_type()
