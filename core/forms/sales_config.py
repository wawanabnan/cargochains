...
class SalesConfigForm(forms.Form):
    ...
    default_currency = forms.ModelChoiceField(
        queryset=None,  # nanti set di __init__ (biar tidak import circular)
        required=False,
        widget=forms.Select(attrs={"class": "form-select form-select-sm js-focus-select"})
    )
    ...
    # tampil untuk user (format Indonesia). nilai simpan dari hidden raw.
    sales_fee_percent = forms.CharField(
        required=False,
        widget=forms.TextInput(attrs={"class": "form-control form-control-sm js-fee", "placeholder": "0,00"})
    )

    # ✅ TinyMCE fields (INI YANG HILANG)
    customer_notes = forms.CharField(
        required=False,
        widget=forms.Textarea(attrs={"class": "form-control form-control-sm js-tinymce", "rows": 8})
    )
    sla = forms.CharField(
        required=False,
        widget=forms.Textarea(attrs={"class": "form-control form-control-sm js-tinymce", "rows": 8})
...
def __init__(self, *args, **kwargs):
    ...
    self.fields["default_currency"].queryset = Currency.objects.all().order_by("code")

    # dynamic labels dari file JSON
    labels = load_sales_labels()

    # ---------- TAX MODE (label + help) ----------
    mode_cfg = labels.get("tax_mode") or {}

    def _mode_label(key: str, fallback: str) -> str:
        v = mode_cfg.get(key) or {}
        return (v.get("label") or fallback) if isinstance(v, dict) else fallback

    def _mode_help(key: str, fallback: str = "") -> str:
        v = mode_cfg.get(key) or {}
        return (v.get("help") or fallback) if isinstance(v, dict) else fallback

    # ✅ choices label diambil dari JSON (kalau ada), fallback English kalau tidak ada
    ...
    # ✅ help text juga diambil dari JSON (kalau ada)
    self.tax_mode_help = {
    ...
    def _apply_label(key: str, fallback: str) -> str:
        v = apply_cfg.get(key)
